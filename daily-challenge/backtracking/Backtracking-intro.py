# Today I'd like to give you some tips on how to solve backtracking questions.
...
# This is called pruning. We must be able to test partial solutions:
# for example, we can't find a global optimum using backtracking, since we have no idea if the solution
# we're currently on can lead to it or not. But we can, for example, solve Sudoku using backtracking.
# We can know immediately if our solution so far is invalid by testing if two of the same number appear in the same row, column, or square.
...
# First off, let's describe the brute force solution to this problem, which means trying every single combination 
# of N queens in each of N * N spots. That's n2 choose n, which is painfully slow. We can immediately
# improve the runtime of this algorithm by noticing that there's no point in ever placing two queens on the same row (or column),
# so we really only need to have one queen per row. Now, using brute force, we need to iterate over each row and over each spot on each row.
# Since we have N rows and N columns, our runtime will be O(NN). That's still hella slow, though.
...
# We'll create an is_valid function that will check the board on each incremental addition. is_valid will look
# at the last queen placed and see if any other queen can threaten it. If so, then we prune the branch since there's no point pursuing it.
# Otherwise, we'll recursively call ourselves with the new incremental solution.
...
# where the value at the index i that represents the column the queen on row i is on. Since we're working incrementally,
...
# and a starting airport, compute the person's itinerary. If no such itinerary exists, return null. All flights must be used in the itinerary.
...
# You should take some time to try to solve it on your own! Notice that a greedy solution won't work,
# since it's possible to have a cycle in the graph.
...
# Can we verify if the partial solution is invalid?
# Yes, we can check a solution is invalid if 1) there are no flights leaving from our last destination and 2)
# there are still flights remaining that can be taken. Since we must use all flights, this means we're at a dead end.
...
# Can we verify if the partial solution is invalid?
# Yes, we can check that the board is valid so far if there are no rows, columns,
# or squares that contain the same digit.

# Can we verify if the solution is complete?
# Yes, the solution is complete when the board has been filled up.

# Let's try to solve it using our template. We'll try filling each empty cell one by one,
# and backtrack once we hit an invalid state. That'll look something like this:
